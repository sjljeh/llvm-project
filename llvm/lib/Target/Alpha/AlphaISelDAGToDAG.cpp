...
namespace {
  ...
  class AlphaDAGToDAGISel : public SelectionDAGISel {
    ...
    static uint64_t abs64(int64_t x) { return x < 0 ? -uint64_t(x) : x; }
  ...
  public:
    ...
    inline SDValue getI64Imm(int64_t Imm) {
      return CurDAG->getTargetConstant(Imm, SDLoc(), MVT::i64);
    }
...
private:
    ...
    const AlphaInstrInfo *getInstrInfo() {
      return static_cast<const AlphaInstrInfo *>(MF->getSubtarget().getInstrInfo());
    }
  ...
  public:
    static char ID;
    explicit AlphaDAGToDAGISelLegacy(AlphaTargetMachine &TM)
        : SelectionDAGISelLegacy(ID,
                                 std::make_unique<AlphaDAGToDAGISel>(TM)) {}
  };
}
...
SDNode *AlphaDAGToDAGISel::getGlobalBaseReg() {
  ...
  return CurDAG->getRegister(GlobalBaseReg,
                             TLI->getPointerTy(CurDAG->getDataLayout())).getNode();
}
...
SDNode *AlphaDAGToDAGISel::getGlobalRetAddr() {
  ...
  return CurDAG->getRegister(GlobalRetAddr,
                             TLI->getPointerTy(CurDAG->getDataLayout())).getNode();
}
...
void AlphaDAGToDAGISel::Select(SDNode *N) {
  ...
  case AlphaISD::DivCall: {
    ...
    if (getTargetMachine().getTargetTriple().isOSBinFormatCOFF()) {
      Chain = CurDAG->getCopyToReg(Chain, dl, Alpha::R16, N1, SDValue(0, 0));
      Chain = CurDAG->getCopyToReg(Chain, dl, Alpha::R17, N2,
                                   Chain.getValue(1));
      Chain = CurDAG->getCopyToReg(Chain, dl, Alpha::R27, N0,
                                   Chain.getValue(1));
      SDNode *CNode = CurDAG->getMachineNode(
          Alpha::JSR, dl, MVT::Other, MVT::Glue, Chain, Chain.getValue(1));
      Chain = CurDAG->getCopyFromReg(SDValue(CNode, 0), dl, Alpha::R0,
                                     MVT::i64, SDValue(CNode, 1));
      CurDAG->SelectNodeTo(N, Alpha::BISr, MVT::i64, Chain, Chain);
      return;
    }
  ...
  case ISD::READCYCLECOUNTER: {
    ...
    ReplaceNode(N, CurDAG->getMachineNode(Alpha::RPCC, dl, MVT::i64,
                                          MVT::Other, Chain));
    return;
  }
  ...
  case ISD::Constant: {
    ...
    if (val <= IMM_HIGH + IMM_HIGH * IMM_MULT &&
      ...
      break; //(LDAH (LDA))
    if ((uval >> 31) == 0 && //empty upper bits after signed 32-bit materialize
    ...
    SDValue Ops[] = {CPI, SDValue(Tmp, 0), CurDAG->getEntryNode()};
    CurDAG->SelectNodeTo(N, Alpha::LDQr,
                         CurDAG->getVTList(MVT::i64, MVT::Other), Ops);
    return;
  }
  ...
  case ISD::ConstantFP: {
    ...
    if (CN->getValueAPF().isPosZero()) {
      CurDAG->SelectNodeTo(N, isDouble ? Alpha::CPYST : Alpha::CPYSS,
                           T, CurDAG->getRegister(Alpha::F31, T),
                           CurDAG->getRegister(Alpha::F31, T));
      return;
    } else if (CN->getValueAPF().isNegZero()) {
      CurDAG->SelectNodeTo(N, isDouble ? Alpha::CPYSNT : Alpha::CPYSNS,
                           T, CurDAG->getRegister(Alpha::F31, T),
                           CurDAG->getRegister(Alpha::F31, T));
      return;
    } else {
  ...
  case ISD::AND: {
    ...
    if (N->getOperand(0).getOpcode() == ISD::SRL &&
      ...
      if (get_zapImm(mask)) {
        ...
        ReplaceNode(N, CurDAG->getMachineNode(Alpha::SRLr, dl, MVT::i64, Z,
                                              getI64Imm(sval)));
        return;
      }
    }
...
FunctionPass *llvm::createAlphaISelDag(AlphaTargetMachine &TM) {
  return new AlphaDAGToDAGISelLegacy(TM);
}
