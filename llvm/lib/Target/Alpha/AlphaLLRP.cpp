  ...
  struct AlphaLLRPPass : public MachineFunctionPass {
    ...
    bool runOnMachineFunction(MachineFunction &F) {
      ...
      for (MachineFunction::iterator FI = F.begin(), FE = F.end();
        ...
        bool NextIsBranchTarget = false;
        if (MachineBasicBlock *NextMBB = MBB.getNextNode()) {
          for (MachineBasicBlock *Pred : NextMBB->predecessors()) {
            if (Pred != &MBB) {
              NextIsBranchTarget = true;
              break;
            }
          }
        }

        if (ub || NextIsBranchTarget || AlignAll) {
          //we can align stuff for free at this point
          while (count % 4) {
            BuildMI(MBB, MBB.end(), dl, TII->get(Alpha::UNOP));
            ++count;
            ++nopalign;
            Changed = true;
            prev[0] = prev[1];
