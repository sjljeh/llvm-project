...
// RUN: %clang_cc1 %s --dependent-lib=msvcrt -triple powerpcle-pc-windows-msvc -emit-llvm -o - | FileCheck %s
// RUN: %clang_cc1 %s --dependent-lib=msvcrt -triple powerpcle-pc-windows-msvc -emit-obj -o %t
// RUN: llvm-readobj --coff-directives %t | FileCheck %s --check-prefix=COFF
...
// COFF: Directive(s): /DEFAULTLIB:msvcrt.lib
