...
// CHECK-LABEL: define dso_local {{.*}}void @try_body(i32 noundef {{(signext )?}}%numerator, i32 noundef {{(signext )?}}%denominator, ptr noundef %myres)
// CHECK: sdiv i32
// CHECK: store i32 %{{.*}}, ptr
// CHECK: ret void
...
// CHECK-LABEL: define dso_local {{.*}}i32 @safe_div(i32 noundef {{(signext )?}}%numerator, i32 noundef {{(signext )?}}%denominator, ptr noundef %res)
...
// CHECK: invoke {{.*}}void @try_body(i32 noundef {{(signext )?}}%{{.*}}, i32 noundef {{(signext )?}}%{{.*}}, ptr noundef %{{.*}}) #[[NOINLINE:[0-9]+]]
...
// RISCV64: %[[padtoken:[^ ]*]] = catchpad within %{{[^ ]*}} [ptr null]
...
// RISCV64: call void (...) @llvm.localescape(ptr %[[r:[^ ,]*]])
...
//
// RISCV64-LABEL: define internal {{.*}}i32 @"?filt$0@0@filter_expr_capture@@"(ptr noundef %exception_pointers, ptr noundef %frame_pointer)
// RISCV64: %[[fp:[^ ]*]] = call ptr @llvm.eh.recoverfp(ptr @filter_expr_capture, ptr %frame_pointer)
// RISCV64: call ptr @llvm.localrecover(ptr @filter_expr_capture, ptr %[[fp]], i32 0)
//
...
//
// CHECK: [[except_inner]]
// CHECK: store {{volatile }}i32 123, ptr %[[r]]
// CHECK: br label %[[inner_try_cont:[^ ]*]]
//
// CHECK: [[inner_try_cont]]
// CHECK: {{invoke|call}}{{.*}}void @llvm.seh.try.end()
//
// CHECK: [[cswitch_outer]]
// CHECK: %[[cs_outer:[^ ]*]] = catchswitch within none [label %[[cpad_outer:[^ ]*]]] unwind to caller
//
// CHECK: [[cpad_outer]]
// CHECK: catchpad within %[[cs_outer]] [ptr @"?filt$0@0@nested_try@@"]
...
// CHECK-LABEL: define dso_local {{.*}}i32 @basic_finally(i32 noundef {{(signext )?}}%g)
// X64-SAME: personality ptr @__C_specific_handler
// ARM64-SAME: personality ptr @__C_specific_handler
// ARM-SAME: personality ptr @__C_specific_handler
// X86-SAME: personality ptr @_except_handler3
...
//
// CHECK: [[cont]]
// CHECK: store {{volatile }}i32 %[[tmp]], ptr %[[rv]]
// CHECK: br label %[[retbb]]
//
// CHECK: [[retbb]]
...
// CHECK:         [[LOCAL:%.*]] = call ptr @llvm.localaddress()
...
// CHECK: invoke {{.*}}void @try_body(i32 noundef {{(signext )?}}0, i32 noundef {{(signext )?}}0, ptr noundef null)
// CHECK: %[[pad:[^ ]*]] = catchpad
